...
def analyze_experiment(experiment_type, variable_name, 
                       logs_base_dir="logs/sweep", 
                       cols=3, figsize_per_row=(15, 3)):

    pattern = os.path.join(logs_base_dir, experiment_type, variable_name, "*.csv")

    plot_multiple_results(pattern, cols=cols, figsize_per_row=figsize_per_row, save_dir=f"plots/{experiment_type}", save_name=f"{variable_name}.png")

    export_metrics_to_json(pattern, output_dir=f"json_output/{experiment_type}/{variable_name}")
